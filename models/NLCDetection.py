...
# moe = MoEAttention(
#     dim=..., attn=..., num_experts=8, hidden_dim=...,
#     return_routing=False,
#     lb_weight=0.05,     # 先从 0.01~0.1 试
#     ent_weight=0.005,   # 先从 0.001~0.01 试
#     temp=1.0,
#     topk=2,
# )
...
class MoEAttention(nn.Module):
    def __init__(
        ...
        topk: int = 32,
        # ===== 新增：路由正则权重 =====
        lb_weight: float = 0.05,     # 负载均衡损失权重（建议 1e-2 ~ 1e-1 起试）
        ent_weight: float = 0.005,    # 熵正则权重（建议 1e-3 ~ 1e-2 起试）
        noise_std: float = 0.01,    # 训练期门控噪声标准差
        eps: float = 1e-9,
    ):
        super().__init__()
        ...
        # 负载均衡统计: 每个 expert 的平均 routing 概率（仅监控用，不参与梯度）
    ...
    def _build_routing(self, x: torch.Tensor):
        ...
        if self.training:
            ...
            routing = F.softmax(gate_logits.float(), dim=1).to(gate_logits.dtype)
        else:
            ...
            routing = routing.to(topk_probs.dtype)
            routing.scatter_(1, topk_idx, topk_probs)

        return routing, gate_logits

    # ---------------- routing regularization (NEW) ----------------
    def _routing_regularization(self, routing: torch.Tensor):
        """
        routing: [B,E,H,W] (训练期 dense soft routing)
        返回:
          lb_loss: 负载均衡损失（让专家平均被使用）
          ent_reg: 熵正则（鼓励路由分布不要过早变得极端）
        """
        B, E, H, W = routing.shape

        # 1) 负载均衡：专家平均使用率接近 1/E
        usage = routing.mean(dim=(0, 2, 3))  # [E], 可微！
        target = torch.full_like(usage, 1.0 / E)
        lb_loss = ((usage - target) ** 2).sum()

        # 2) 熵正则：鼓励高熵（更探索），训练时最小化 -entropy
        # entropy = -sum_e p log p；我们返回 ent_reg = -entropy（越小越好 => entropy 越大）
        entropy = -(routing * (routing + self.eps).log()).sum(dim=1).mean()  # scalar
        ent_reg = -entropy

        return lb_loss, ent_reg

    # ---------------- forward ----------------
    def forward(
        self,
        x: torch.Tensor,
        return_expert_maps: bool = False,
        return_aux_loss: bool = True,   # ===== 新增：返回路由正则项 =====
    ):
        """
        x: [B,C,H,W]
        ...
        若 return_aux_loss=True:
          额外返回 aux_loss (scalar)，用于加到总 loss 中
        ...
        # 2) 统计每个 expert 的平均使用率(监控用，不影响梯度)
        with torch.no_grad():
        ...
        # 5) 多样性(基于 expert_masks) —— 监控用
        self.diversity = expert_masks.var(dim=1).mean().item()

        # 6) 路由正则/负载均衡（NEW）
        aux_loss = None
        if return_aux_loss:
            if self.training and (self.lb_weight > 0.0 or self.ent_weight > 0.0):
                lb_loss, ent_reg = self._routing_regularization(routing)  # 注意：routing 不能 detach
                aux_loss = self.lb_weight * lb_loss + self.ent_weight * ent_reg
            else:
                # 推理期或权重为0时，返回0，方便外部直接相加
                aux_loss = torch.zeros((), device=x.device, dtype=x.dtype)

        # print("expert_masks:", expert_masks.min().item(), expert_masks.max().item())
        # print("routing:", routing.min().item(), routing.max().item(), routing.sum(dim=1).mean().item())

        # 7) debug 缓存：不改变原有返回也能取到
        ...
        if self.return_routing:
            outs.append(routing)
        if return_aux_loss:
            outs.append(aux_loss)
        if return_expert_maps:
            outs.append(debug)

        return tuple(outs) if len(outs) > 2 else (mask, res)
...
class NLCDetection(nn.Module):
  def __init__(self, args):
    ...
    # self.getmask4 = MoEAttention(feat4_num, CBAM, 8, self.crop_size, topk=5)
    # self.getmask3 = MoEAttention(feat3_num, CBAM, 8, self.crop_size, topk=5)
    # self.getmask2 = MoEAttention(feat2_num, CBAM, 8, self.crop_size, topk=5)
    # self.getmask1 = MoEAttention(feat1_num, CBAM, 8, self.crop_size, topk=5)
  ...
  def forward(self, feat):
    ...
    if self.aux_loss:
        mask4, feat4, aux_loss4 = self.getmask4(s4)
        mask4U = F.interpolate(
        mask4, size=s3.size()[2:], mode='bilinear', align_corners=True)

        s3 = s3 * mask4U
        mask3, feat3, aux_loss3 = self.getmask3(s3)
        mask3U = F.interpolate(
        mask3, size=s2.size()[2:], mode='bilinear', align_corners=True)

        s2 = s2 * mask3U
        mask2, feat2, aux_loss2 = self.getmask2(s2)
        mask2U = F.interpolate(
        mask2, size=s1.size()[2:], mode='bilinear', align_corners=True)

        s1 = s1 * mask2U
        mask1, feat1, aux_loss1 = self.getmask1(s1)

        return [mask1, mask2, mask3, mask4], [feat1, feat2, feat3, feat4], [aux_loss1, aux_loss2, aux_loss3, aux_loss4]
    else:
