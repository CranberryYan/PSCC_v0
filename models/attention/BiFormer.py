...
class BiLevelRoutingAttention(nn.Module):
    ...
    def __init__(self, dim, n_win=7, num_heads=8, qk_dim=None, qk_scale=None,
        ...
        self.getmask = nn.Sequential(
            nn.Conv2d(in_channels=dim, out_channels=16,
    ...
    def forward(self, x, ret_attn_mask=False):
        ...
        mask = torch.sigmoid(self.getmask(out.clone()))
...
def main():
    ...
    width = 14
    num_heads = 4
    n_win = 7  # number of windows per side

    # create dummy input: (N, C, H, W)
    ...
    # forward pass with returning intermediate shapes
    out = attn(x, ret_attn_mask=false)

    # print the shapes
    print(f"Output tensor shape: {out.shape}")
    # print(f"Routing weights shape: {r_weight.shape}")  # (N, n_win^2, topk)
    # print(f"Routing indices shape: {r_idx.shape}")    # (N, n_win^2, topk)
    # print(f"Attention mask shape: {attn_mask.shape}") # (N*n_win^2, num_heads, win_size^2, topk*downsample_size^2)
