...
class HiLo(nn.Module):
  ...
  def hifi(self, x):
    ...
    qkv = self.h_qkv(x).reshape(B, total_groups, -1, 3,
            self.h_heads, self.h_dim // self.h_heads).permute(3, 0, 1, 4, 2, 5)
  ...
  def lofi(self, x):
    ...
    q = self.l_q(x).reshape(B, H * W, self.l_heads,
                            self.l_dim // self.l_heads).permute(0, 2, 1, 3)

    if self.ws > 1:
      x_ = x.permute(0, 3, 1, 2)
      x_ = self.sr(x_).reshape(B, C, -1).permute(0, 2, 1)
      kv = self.l_kv(x_).reshape(B, -1, 2,
                          self.l_heads,
                          self.l_dim // self.l_heads).permute(2, 0, 3, 1, 4)
    else:
      kv = self.l_kv(x).reshape(B, -1, 2, self.l_heads,
                          self.l_dim // self.l_heads).permute(2, 0, 3, 1, 4)
    k, v = kv[0], kv[1]
  ...
  def flops(self, H, W):
    # pad the feature map when the height and
    #   width cannot be divided by window size
...
if __name__ == '__main__':
  block = HiLo(dim=128)
  input = torch.rand(4, 16*8, 128) # input with shape (B, N, C)
