...
def compute_f1_scores(
    ...
    """
    - 如果 GT mask 不存在 → 视为原图，自动生成全黑 mask
    - 所有样本统一参与像素级 P / R / F1 评估
    """
    ...
    def _process_one(idx_line):
        ...
        # GT mask：fake → mask，authentic → 全黑
        ...
        return idx, (line, prec, rec, f1, tp, fp, fn, tag, is_authentic), None
    ...
    for _, (line, p, r, f1, tp, fp, fn, tag, is_auth) in results:
        micro_tp += tp
        micro_fp += fp
        micro_fn += fn
        ps.append(p); rs.append(r); fs.append(f1)
        out_lines.append(f"{line}\tP:{p:.4f}\tR:{r:.4f}\tF1:{f1:.4f}\n")
    ...
    with open(save_path, 'w', encoding='utf-8') as f:
        ...
        if fs:
            f.write("\n")
            f.write(f"Average P/R/F1: {np.mean(ps):.4f} / {np.mean(rs):.4f} / {np.mean(fs):.4f}\n")
            ...
            f.write(f"Micro   P/R/F1: {mp:.4f} / {mr:.4f} / {mf:.4f}\n")
